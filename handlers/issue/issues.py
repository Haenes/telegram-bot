...
@router.callback_query(F.data.startswith("issue_"), F.data.as_("data"), flags={"set_headers":"set_headers"})
async def send_issue(callback: types.CallbackQuery, data, user_headers):
    issue_id = data.removeprefix("issue_")
    results = get_issue(issue_id, user_headers)
